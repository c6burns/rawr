...
static void srv_websock_recv_handler(const struct websock_hdr *hdr, struct mbuf *mb, void *arg)
{
    struct websocket *ws = arg;
    int err;

    err = websock_send(ws->conn, hdr->opcode, "%b", mbuf_buf(mb), mbuf_get_left(mb));
    if (err)
...
static void http_req_handler(struct http_conn *conn, const struct http_msg *msg, void *arg)
{
    ...
    err = websock_accept(&ws->conn, server, conn, msg, 0, srv_websock_recv_handler, srv_websock_close_handler, ws);
    if (err) {
