...
void rtp_session_run(program_type prog_type, const char *address, uint16_t local_port, uint16_t remote_port)
{
...
#endif
    srtp_sec_serv_t sec_servs = sec_serv_conf_and_auth;
    unsigned char ttl = 5;
    int c;
    int key_size = 256;
    int tag_size = 8;
    ...
    int expected_len;
    int do_list_mods = 0;
    uint32_t ssrc0 = 0xdeadbea7;
    uint32_t ssrc1 = 0xdeedbee7;
    uint32_t ssrc_local, ssrc_remote;

    if (prog_type == sender) {
        input_key = "GYIFI12UYMWkIVgGYAZzMZJWEJI4VgEpNYYQlYAYHYIFI12UYMWkIVgGYAZzMZJWEJI4VgEpNYYQlnrn";
        ssrc_local = ssrc0;
        ssrc_remote = ssrc1;
    } else if (prog_type == receiver) {
        input_key = "HYIFI12UYMWkIVgGYAZzMZJWEJI4VgEpNYYQlnrnGYIFI12UYMWkIVgGYAZzMZJWEJI4VgEpNYYQlYAY";
        ssrc_local = ssrc1;
        ssrc_remote = ssrc0;
    } else {
    ...
    if (sec_servs && !input_key) {
        mn_log_error("required key not provided");
        return;
    }
...
/* set address */
    ...
    if (prog_type == sender) {
        ...
        rtp_sender_init(snd, sock, name, ssrc_local);
        ...
        rtp_receiver_init(rcvr, sock, name, ssrc_remote);
...
{
    rtp_session_run(sender, "192.168.86.105", 22451, 22452);
