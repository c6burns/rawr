...
/**
 * Add a SIP transport with an external IP contact
 *
 * @param sip   SIP stack instance
 * @param tp    SIP Transport
 * @param ext_addr External network address
 * @param laddr Local network address
 * @param ...   Optional transport parameters such as TLS context
 *
 * @return 0 if success, otherwise errorcode
 */
int sip_transp_add_ext(struct sip *sip, enum sip_transp tp, const struct sa *ext_addr, const struct sa *laddr, ...)
{
    struct sip_transport *transp;
    struct tls *tls;
    va_list ap;
    ...
    case SIP_TRANSP_UDP:
        ...
        udp_local_get(transp->sock, &transp->laddr);
        if (!sa_port(ext_addr)) {
            sa_set_port(ext_addr, sa_port(&transp->laddr));
        }
        sa_cpy(&transp->laddr, ext_addr);
    ...
    case SIP_TRANSP_TCP:
        err = tcp_listen((struct tcp_sock **)&transp->sock, laddr, tcp_connect_handler, transp);
        if (err)
            break;
