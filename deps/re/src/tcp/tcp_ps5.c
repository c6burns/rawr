...
{
    struct tcp_qent *qe = (struct tcp_qent *)arg;
...
static int enqueue(struct tcp_conn *tc, struct mbuf *mb)
{
	...
	qe = (struct tcp_qent *)mem_zalloc(sizeof(*qe), qent_destructor);
	if (!qe)
		return ENOMEM;
...
{
    struct tcp_qent *qe = (struct tcp_qent *)list_ledata(tc->sendq.head);
	ssize_t n;
#ifdef MSG_NOSIGNAL
...
static void tcp_recv_handler(int flags, void *arg)
{
	...
	if (flags & FD_WRITE) {
		...
		le = tc->helpers.head;
		while (le) {
            struct tcp_helper *th = (struct tcp_helper *)le->data;
...
int tcp_sock_bind(struct tcp_sock *ts, const struct sa *local)
{
	...
	err = EINVAL;
    if (bind(ts->fd, (struct sockaddr *)local, sizeof(*local)) < 0) {
		err = errno;
        return err;
	}
		   ...
		   tcp_recv_h *rh, tcp_close_h *ch, void *arg)
{
	...
	tc->fdc = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (tc->fdc < 0) {
		err = errno;
        return err;
	}
...
int tcp_conn_bind(struct tcp_conn *tc, const struct sa *local)
{
	...
	err = net_hostaddr(AF_INET, &hostaddr);
	if (err < 0) {
        return -1;
	}

	/* bind to local address */
	if (bind(tc->fdc, (struct sockaddr *)&hostaddr, sizeof(hostaddr)) < 0) {
...
again:
	if (0 == connect(tc->fdc, (struct sockaddr *)peer, sizeof(*peer))) {
		err = 0;
		goto out;
	}
	else {
