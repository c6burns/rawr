...
static void dummy_udp_recv_handler(const struct sa *src, struct mbuf *mb, void *arg)
{
    (void)src;
    (void)mb;
    (void)arg;
...
static bool helper_send_handler(int *err, struct sa *dst, struct mbuf *mb, void *arg)
{
    (void)err;
    (void)dst;
    (void)mb;
...
static void udp_read(struct udp_sock *us, int fd)
{
    ...
    n = recvfrom(fd, BUF_CAST mb->buf + us->rx_presz, mb->size - us->rx_presz, 0, &src.u.sa, &src.len);
    if (n < 0) {
        err = errno;
    ...
    le = us->helpers.head;
    while (le) {
        struct udp_helper *uh = (struct udp_helper *)le->data;
        bool hdld;
...
{
    struct udp_sock *us = (struct udp_sock *)arg;
...
int udp_listen(struct udp_sock **usp, const struct sa *local, udp_recv_h *rh, void *arg)
{
    ...
    if (local) {
        af = sa_af(local);
        (void)re_snprintf(addr, sizeof(addr), "%H", sa_print_addr, local);
    ...
    sa_init(&hostaddr, AF_INET);
    if (net_hostaddr(af, &hostaddr) < 0) {
        return -1;
    }
    ...
    sa_set_in(&hostaddr, SCE_NET_INADDR_ANY, sa_port(local));
    if (bind(fd, (struct sockaddr *)&hostaddr, hostaddr.len) < 0) {
        err = errno;
...
static int udp_send_internal(struct udp_sock *us, const struct sa *dst, struct mbuf *mb, struct le *le)
{
    struct sa hdst;
    int err = 0, fd;
    ...
    if (us->conn) {
        ...
        if (sendto(fd, BUF_CAST mb->buf + mb->pos, mb->end - mb->pos, 0, &dst->u.sa, dst->len) < 0)
            return errno;
    }
...
int udp_setsockopt(struct udp_sock *us, int level, int optname, const void *optval, uint32_t optlen)
{
    ...
    if (-1 != us->fd) {
        if (0 != setsockopt(us->fd, level, optname, BUF_CAST optval, optlen))
            err |= errno;
    }

    if (-1 != us->fd6) {
        if (0 != setsockopt(us->fd6, level, optname, BUF_CAST optval, optlen))
            err |= errno;
    }
 ...
 * @return 0 if success, otherwise errorcode
 */
int udp_register_helper(struct udp_helper **uhp, struct udp_sock *us, int layer, udp_helper_send_h *sh, udp_helper_recv_h *rh, void *arg)
{
 ...
 * @return 0 if success, otherwise errorcode
 */
int udp_send_helper(struct udp_sock *us, const struct sa *dst, struct mbuf *mb, struct udp_helper *uh)
{
    if (!us || !dst || !mb || !uh)
        return EINVAL;
