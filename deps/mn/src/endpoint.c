...
void mn_endpoint_from_short(mn_endpoint_t *endpoint, uint16_t port, uint16_t s0, uint16_t s1, uint16_t s2, uint16_t s3, uint16_t s4, uint16_t s5, uint16_t s6, uint16_t s7)
{
    ...
    mn_endpoint_port_set(endpoint, port);
...
int mn_endpoint_string_get(mn_endpoint_t *endpoint, uint16_t *port, char *buf, int buf_len)
{
    ...
    *port = 0;
    ...
    if (mn_endpoint_is_ipv6(endpoint)) {
        uv_ip6_name(sa, ipbuf, 255);
        *port = ntohs(sa->sin6_port);
    } else if (mn_endpoint_is_ipv4(endpoint)) {
...
int mn_endpoint_from_string(mn_endpoint_t *endpoint, const char *ip, uint16_t port)
{
    MN_ASSERT(endpoint);
    MN_ASSERT(ip);
    if (mn_endpoint_set_ip6(endpoint, ip, port)) {
        return mn_endpoint_set_ip4(endpoint, ip, port);
    }
...
int mn_endpoint_set_ip4(mn_endpoint_t *endpoint, const char *ip, uint16_t port)
{
    ...
    if (!endpoint) return MN_ERROR;
    if (!ip) return MN_ERROR;

    memset(endpoint, 0, sizeof(*endpoint));
    if ((ret = uv_ip4_addr(ip, port, (struct sockaddr_in *)endpoint))) return MN_ERROR;
...
int mn_endpoint_set_ip6(mn_endpoint_t *endpoint, const char *ip, uint16_t port)
{
    ...
    memset(endpoint, 0, sizeof(*endpoint));
    if ((ret = uv_ip6_addr(ip, port, (struct sockaddr_in6 *)endpoint))) return MN_ERROR;

    return MN_SUCCESS;
}

// --------------------------------------------------------------------------------------------------------------
int mn_endpoint_convert_from(mn_endpoint_t *endpoint, void *sockaddr_void)
{
    if (!endpoint) return MN_ERROR;
    if (!sockaddr_void) return MN_ERROR;
...
{
    mn_endpoint_t *addr = (mn_endpoint_t *)sockaddr;
    if (mn_endpoint_af_get(addr) != mn_endpoint_af_get(endpoint)) return false;

    if (mn_endpoint_is_ipv4(addr)) {
        ...
        return true;
    } else if (mn_endpoint_is_ipv6(addr)) {
        mn_sockaddr4_t *ep_addr4 = (mn_sockaddr4_t *)endpoint;
        mn_sockaddr4_t *addr4 = (mn_sockaddr4_t *)addr;
